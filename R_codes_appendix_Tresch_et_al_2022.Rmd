---
title: "R script for the paper *The cumulative impacts of climate change induced droughts and air pollutants on
the decline of Norway spruce (Picea abies) in Switzerland based on 37 years of
forest monitoring* by Tresch *et* al. 2022"
...
All codes are coded by the main authors, based on literature given in the main paper and supplementary material. Some R codes for big plots are not displayed in the PDF output but they can be found in the .RMD file.
...
# 131814 trees have been observed 1985-2021.

sum(mydata$trees_died) 
#In total 1183 trees died from 1985-2021.
...
(long_term_average_mortality<-mean(mortality_rate$mortality, na.rm=T)) #  0.71 % 1985-2021
```
...
mortality_rate_plot <-ggplot(data=mortality_rate,aes(x=year_fact, y=mortality/100)) + 
  ...
  annotate("text", x = 1994.3, y = 0.0088, size = 2.2,  colour="grey20", lineheight =
             .9,label = "Long-term avarage 1985-2021: 0.71%",hjust = 1)+
...
n_dep_hist<-ggplot(ndep_data, aes(y=n_dep, x=sites, colour=year_fact)) +
 ...
 annotate("text", x = 23, y = 27, size = 2.8,  colour="grey20", lineheight = .9,label = expression(paste("Median 1985-2021: 26 kg N ", ha^-1, " ", a^-1)),hjust = 1)+
...
drought_plot<-ggplot(drought_plot_data,aes(y=measurements,x=as.factor(as.character(year_fact)),colour=groups)) +
  scale_x_discrete(breaks = c(1985:2021,1))+
...
# elpd_diff se_diff
# brms_poly_lags_drought     0.0       0.0  
# brms_poly_lags_drought_1 -44.5      24.7  
# brms_poly_lags_drought_3 -59.9      37.6  
# brms_poly_lags_drought_2 -62.9      38.9 

# brms_poly_lags_drought best model

# waic(brms_poly_lags_drought)
# Estimate    SE
# elpd_waic  -1395.9 122.1
# p_waic       242.9  46.7
# waic        2791.7 244.2

# waic(brms_poly_lags_drought_1)
# Estimate    SE
# elpd_waic  -1434.4 128.2
# p_waic       259.8  50.4
# waic        2868.7 256.4

# waic(brms_poly_lags_drought_2)
# Estimate    SE
# elpd_waic  -1449.8 128.8
# p_waic       255.9  47.8
# waic        2899.5 257.5
...
# Estimate    SE
# elpd_waic  -1448.9 126.5
# p_waic       250.8  46.0
# waic        2897.8 253.0

# lagged effects plot of best poly lag model 
model <- brms_poly_lags_drought
...
# with 6 lags (lag 0- lag5) and 4 u (u0-u3):
(w<-rep(c(5,4,3,2,1,0),4)) #lag 0 - lag5  & u0, u1, u2, u3
(e<-c(rep(0,6),rep(1,6),rep(2,6),rep(3,6))) #lag 0 - lag5  & u0, u1, u2, u3


w_<-w^e

 dim(w_)<-c(6,4) #lag 0 - lag5  & u0, u1, u2, u3
...
lag_effects<-     as.data.frame(t(beta$u0+ (beta$u1*(0^1))+ (beta$u2*(0^2))+ (beta$u3*(0^3)) ))
lag_effects$lag1<-as.data.frame(t(beta$u0+ (beta$u1*(1^1))+ (beta$u2*(1^2))+ (beta$u3*(1^3)) ))
lag_effects$lag2<-as.data.frame(t(beta$u0+ (beta$u1*(2^1))+ (beta$u2*(2^2))+ (beta$u3*(2^3)) ))
lag_effects$lag3<-as.data.frame(t(beta$u0+ (beta$u1*(3^1))+ (beta$u2*(3^2))+ (beta$u3*(3^3)) ))
lag_effects$lag4<-as.data.frame(t(beta$u0+ (beta$u1*(4^1))+ (beta$u2*(4^2))+ (beta$u3*(4^3)) ))
lag_effects$lag5<-as.data.frame(t(beta$u0+ (beta$u1*(5^1))+ (beta$u2*(5^2))+ (beta$u3*(5^3)) ))
...
lag_effects_matrix$lags<-c("lag 5 years","lag 4 years","lag 3 years","lag 2 years","lag 1 year",
                           "no lag effect")
...
# drought lagged arithmetic mean
mydata$drought_lag <- (mydata$ETa_ETp_0 + mydata$ETa_ETp_1 + mydata$ETa_ETp_2 + mydata$ETa_ETp_3+ 
                         mydata$ETa_ETp_4)/5
...
crown_def<- read_delim("data/crown_def_data_spruce_2021.csv")
...
#  spearman’s ranked correlation coefficient -> best correlation R=0.34,p=0.04
...
brms_effect_plot_n_dep<-ggplot(brms_pred,aes(x=effect1__,y=estimate__))+
  ...
  theme(panel.background=element_blank(),
        ...
        legend.justification = c(1, 1), legend.position = c(1,1),legend.direction="horizontal",legend.key.height=unit(1,"line"),
        plot.title = element_text(hjust = 0.5))+
...
brms_pred<-conditional_effects(mod, "p_s")$p_s

brms_pred$effect1__<-(sd(mydata$p,na.rm=T)*brms_pred$effect1__)+mean(mydata$p,na.rm=T) 
# un-scaling z-transf

brms_effect_plot_p<-ggplot(brms_pred,aes(x=effect1__,y=estimate__))+
  geom_ribbon(aes(ymin = lower__, ymax = upper__), alpha = .1,show.legend = F,size=0.0001) +
  geom_line(size=1.5)+scale_y_continuous( limits=c(0,0.048),
                                          labels = scales::percent_format(accuracy = 1))+
  geom_vline(aes(xintercept  =1.30), linetype="dashed", size=0.5)+
  #lower threshold for normal nutrition Gottlein2016
labs(x=expression(paste("Foliar phosphorus content (mg ",g^{-1}," dm)")),
     y = "Mortality Picea abies")+
  theme(axis.title=element_text(size=12),axis.ticks = element_line(size=0.1), 
        axis.line=element_line(colour="black"),text = element_text(size=14, colour = "black"),
        axis.text.y = element_text(size = 12, colour = "black"),
...
### effect plot year_s
```{r brms11, echo = T, message=F,warning=F,fig.align = "center"}

brms_pred<-conditional_effects(mod, "year_s")$year_s

 brms_pred$effect1__<-(sd(as.numeric(as.character(mydata$year)))*brms_pred$effect1__)+
   mean(as.numeric(as.character(mydata$year))) # un-scaling z-transf

brms_effect_plot_year<-ggplot(brms_pred,aes(x=effect1__,y=estimate__))+
  geom_ribbon(aes(ymin = lower__, ymax = upper__), alpha = .1,show.legend = F,size=0.0001) +
     geom_line(size=1.5)+scale_y_continuous( limits=c(0,0.048),
                          labels = scales::percent_format(accuracy = 1))+
  labs( x="Year",y = "Mortality Picea abies")+
  theme(axis.title=element_text(size=12),axis.ticks = element_line(size=0.1), 
        axis.line=element_line(colour="black"),text = element_text(size=14, colour = "black"),
        axis.text.y = element_text(size = 12, colour = "black"),
        axis.text.x = element_text(size =12, colour = "black"),
...
#  mydata$Ndep <- mydata$n_dep #Time series data on total N deposition were based on results of the EMEP MSC-W model (0.1° resolution) (EMEP, 2019). 
...
#  mydata$fixeff3 <-(sd(mydata$drought_lag_mean)*(mydata$drought_lag_poly*-1))+mean(mydata$drought_lag_mean) # un-scaling z-transf
...
#    trees_died | trials(n_trees) ~ beta0 + beta1 * fixeff1 + beta2 * fixeff2 + beta3 * fixeff3 + beta4 * fixeff4 + beta5 * fixeff5 + beta6 * fixeff6 +
...
brms_pred<- brms::conditional_effects(CPR_brms, "Ndep:fixeff3",int_conditions = list(fixeff3=c( 0.02,-0.03,-0.13)))$Ndep
...
brms_effect_plot_ClempN_drought_lag<-ggplot(brms_pred,aes(x=effect1__,y=estimate__,color=effect2__,fill=effect2__))+
  geom_ribbon(aes(ymin = lower__, ymax = upper__), alpha = .1,show.legend = F,size=0.0001) +
  geom_line(size=1.5)+
   scale_y_continuous( labels = scales::percent_format(accuracy = 1),breaks=c(0,0.02,0.04,0.06,0.08,0.1))+
   scale_x_continuous( breaks=c(0,10,20,30,40,50,60,70,80))+
  
  geom_segment(aes(x = 9.47 , y = -1, xend = 9.47, yend = 0.078),linetype="solid",size=0.5, colour="grey20")+
  geom_segment(aes(x = 2.23, y = -1, xend = 2.23, yend = 0.078),linetype="dotted",size=0.5, colour="grey20")+
  geom_segment(aes(x = 16.28 , y = -1, xend = 16.28, yend = 0.078),linetype="dotted",size=0.5, colour="grey20")+
  annotate("text", x = 15.4, y = 0.08, size = 2.8,  colour="grey20",label = expression(paste("CLempN estimation")),hjust = 1)+
  
  labs(y="Mortality Picea abies", x=expression(paste("N deposition (kg N ", ha^-1, " ", a^-1, ")")),col=expression("Drought (ETa/ETp)"))+
  scale_fill_manual(values=c(col.4,col.2,col.1))+
  scale_colour_manual(values=c(col.4,col.2,col.1), label=c("0.9","0.85","0.75"))+
  ...
  theme(legend.justification = c(1, 1), legend.position = c(0.9998,0.99),legend.direction="vertical")+
  theme(legend.title=element_text(size=10),legend.box.just = "left")+
 coord_cartesian(ylim = c(0.002, 0.08),
                 xlim= c(0,60)) # Zoom without cutting values
...
mydata<-mydata %>%
  mutate(Ndep_class=case_when(n_dep < 15 ~ "0-15",
                             n_dep >= 15 & n_dep < 20.01 ~ "15-20",
                             n_dep >= 20.01 & n_dep < 25.01 ~ "20-25",
                             n_dep >= 25.01 & n_dep < 30.01 ~ "25-30",
                             n_dep >= 30.01 ~ ">30")) %>%
  mutate(Ndep_fact=as.factor(as.character(Ndep_class)))

# # reorder factor levels for plot
mydata<-mydata %>%
  mutate(Ndep_fact=factor(Ndep_fact, levels= c("0-15","15-20","20-25"
  ,"25-30",  ">30")))
...
# Bayesian change-point regression model with brms for a critical altitude
Concerning the applied method of hierarchical change-point regression models including random effects to estimate empirical critical loads for nitrogen using Bayesian Regression Models (brms) see [@Roth2022a].


The time for fitting one the Bayesian generalised multivariate multilevel binomial regression models can be several hours, depending on the computer power. However the code is given for fitting the models, as # comments.

```{r BCRalit, echo = T, message=F,warning=F,fig.align = "center"}


mydata<- mydata%>% group_by(sites, year)%>%
 mutate (n_trees= sum(trees_died,trees_living)) %>% ungroup()
 mydata$alti <- mydata$altitude # Altitude of sites
 mydata$random<- as.integer(factor(mydata$sites)) #random effect sites

 # #binomial only Calti random
 # changepoint_model <- brmsformula(
 #   trees_died | trials(n_trees) ~ beta0 + 
 #     step(alti - CAlti) * betaAlti * (alti - CAlti), #steps of CAlti regression
 #   CAlti ~ 1+ (1|random), #group level effect for CAlti
 #   # beta0 ~ 1+ (1|random), #group level effect for beta0
 #   beta0 + betaAlti ~ 1,
 #   nl = T,
 #   family = binomial
 # )
 ...
 # bprior <- 
 #   prior(normal(0, 2), nlpar = "beta0") + 
 #   prior(normal(800, 400), nlpar = "CAlti") + 
 #   prior(normal(0, 2),  nlpar = "betaAlti")#vague priors
 # 
 # 
 # # Model fit with brms::brm using Stan
 # CPR_brms <- brms::brm(changepoint_model, 
 ...
 # # # Save  object to file 
 # saveRDS(CPR_brms,
 #         file = "models/CPR_brms_alti.4")
 
 #read it in again
 CPR_brms<-readRDS(file = "models/CPR_brms_alti.4") 
summary(CPR_brms)
...
## R-squared
```{r BCRalti2, echo = T, message=F,warning=F,fig.align = "center"}
...
```{r BCRalti3, echo = T, message=F,warning=F,fig.align = "center"}
...
## effect plots
```{r BCRalti4, echo = T, message=F,warning=F,fig.align = "center"}
 brms_pred<-conditional_effects(CPR_brms, "alti")$alti
 
 brms_effect_Alti<-ggplot(brms_pred,aes(x=effect1__,y=estimate__))+
   geom_ribbon(aes(ymin = lower__, ymax = upper__), alpha = .1,show.legend = F,size=0.0001) +
   geom_line(size=1.5)+
   scale_y_continuous( labels = scales::percent_format(accuracy = 1),
                       breaks=c(0,0.02,0.04,0.06,0.08,0.1))+
   scale_x_continuous( )+
   labs( x = expression(paste("Altitutde (m)")),y = "Mortality Picea abies")+
   
   geom_segment(aes(x = 527.89 , y = -1, xend =527.89 , yend = 0.078),
                linetype="solid",size=0.5, colour="grey20")+
   geom_segment(aes(x =141.52, y = -1, xend =141.52, yend = 0.078),
                linetype="dotted",size=0.5, colour="grey20")+
   geom_segment(aes(x = 968.77, y = -1, xend = 968.77, yend = 0.078),
                linetype="dotted",size=0.5, colour="grey20")+
   annotate("text", x = 750, y = 0.08, size = 2.8,  colour="grey20"
            ,label = expression(paste("Estimated critical altitude")),hjust = 1)+
   ...
   theme(legend.justification = c(1, 1), legend.position = c(0.315,0.99),
         legend.direction="vertical")+
   theme(legend.title=element_text(size=10),legend.box.just = "left")+
   coord_cartesian(ylim = c(0, 0.1)) # Zoom without cutting values
 brms_effect_Alti
```
\clearpage
## References